...
namespace SCPakTool.Pak
{
    public static class PackageContentReader
    {
        ...
        public static ContentSerializer? GetNewSerializerForType(string type)
        {
            if (!ContentSerializers.TryGetValue(type, out var serializerType))
                return null;
            return Activator.CreateInstance(serializerType) as ContentSerializer;
        }
        ...
        public static void WriteContentToFile(FileStream[] fileStreams, ContentSerializer serializer, PackageFile package, ContentDescriptor descriptor)
        {
            ...
            {
                serializer.Read(package.ContentStream);
